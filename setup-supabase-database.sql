...
-- Run this in your Supabase SQL Editor (https://supabase.com/dashboard/project/hdcsxkxohgtpitfljqhx/sql)

-- Create workflows table first (since quotes references it)
CREATE TABLE IF NOT EXISTS workflows (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID REFERENCES auth.users(id) NOT NULL,
    name TEXT NOT NULL,
    description TEXT,
    quote_id UUID, -- Will add foreign key constraint later
...
-- Add foreign key constraint for quote_id in workflows (circular reference)
...
-- Foreign key constraint for workflow_id will be added after workflows table is created
-- (This is handled automatically since workflows table is created before this constraint is needed)

-- Create indexes for performance
CREATE INDEX IF NOT EXISTS idx_quotes_created_by ON quotes(created_by);
CREATE INDEX IF NOT EXISTS idx_quotes_status ON quotes(status);
CREATE INDEX IF NOT EXISTS idx_quotes_workflow_id ON quotes(workflow_id);
CREATE INDEX IF NOT EXISTS idx_quote_items_quote_id ON quote_items(quote_id);
CREATE INDEX IF NOT EXISTS idx_workflows_quote_id ON workflows(quote_id);
CREATE INDEX IF NOT EXISTS idx_workflows_user_id ON workflows(user_id);
CREATE INDEX IF NOT EXISTS idx_workflow_steps_workflow_id ON workflow_steps(workflow_id);
CREATE INDEX IF NOT EXISTS idx_workflow_steps_order ON workflow_steps(workflow_id, order_num);
...
-- Users can access workflow_steps through workflows they own
