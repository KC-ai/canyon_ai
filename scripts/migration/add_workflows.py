#!/usr/bin/env python3
"""
Add workflows to existing quotes that meet the criteria
"""
import json
import sys
import os
...
async def add_workflows():
    ...
    try:
        ...
        # Find quotes for this user that need workflows
        quotes_to_update = []
        for quote_id, quote_data in quotes_db.items():
            if quote_data.get('user_id') == user_id:
                ...
                if needs_workflow and not has_workflow:
                    ...
                    quotes_to_update.append((quote_id, quote_data))
                print()
        
        # Create workflows for quotes that need them
        for quote_id, quote_data in quotes_to_update:
            print(f"Creating workflow for quote {quote_id[:8]}...")
        ...
        if quotes_to_update:
            ...
            print("Refresh your browser to see the workflow components.")
        else:
